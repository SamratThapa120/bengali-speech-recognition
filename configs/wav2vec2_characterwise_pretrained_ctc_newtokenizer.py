...
from bengali_asr.dataset.tokenizer import BengaliTokenizerCTC
...
class Configs(Base):
    OUTPUTDIR="../workdir/wav2vec2_characterlevel_pretrained_ctcloss_noaugs_newtokenizer"
    ...
    VOCAB_NOSPECIAL = [x for x in VOCAB if x not in ['!', '?', ',', '।', '-', '‘', '’', '"', ';', '–', "'", ':', '/', '.', '“', '”']]
    ...
    def __init__(self,inference_files=None,inference_text=None,use_numpy=False):
        ...
        self.model = Wav2Vec2Base(len(self.VOCAB_NOSPECIAL)+1,
                                ...
                                mask_time_prob=0.05,
                                mask_time_length=8)
        self.tokenizer_train = BengaliTokenizerCTC(self.VOCAB_NOSPECIAL)
        self.tokenizer = BengaliTokenizerCTC(self.VOCAB)
